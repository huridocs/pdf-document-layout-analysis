...
class MarkdownConversionServiceAdapter(MarkdownConversionService):
    ...
    def convert_to_markdown(
        ...
        try:
            ...
            if output_file:
                return self._create_zip_response(markdown_content, extracted_images, output_file)
    ...
    def _create_zip_response(
        self, markdown_content: str, extracted_images: List[ExtractedImage], output_filename: str
    ) -> Response:
        zip_buffer = io.BytesIO()

        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            zip_file.writestr(output_filename, markdown_content.encode("utf-8"))
    ...
    def _process_picture_segment(
        ...
        if extracted_images is not None:
            ...
            extracted_images.append(ExtractedImage(img_buffer.getvalue(), image_name))
            return f"<span id='{segment.id}'></span>\n" + f"<img src='{base_name}_pictures/{image_name}' alt=''>\n\n"
        else:
            save_dir = Path(ROOT_PATH, f"{base_name}_pictures")
            save_dir.mkdir(exist_ok=True)
            save_path = save_dir / image_name
            cropped.save(save_path)
            return f"<span id='{segment.id}'></span>\n" + f"<img src='{save_dir.name}/{image_name}' alt=''>\n\n"
    ...
    def _process_title_segment(self, tokens: List[PdfToken], segment: SegmentBox) -> str:
        if not tokens:
            return ""
        content = tokens[0].content_markdown
        for token in tokens[1:]:
            styled = token.token_style.get_styled_content_markdown(token.content)
    ...
    def _process_regular_segment(
        ...
        if not tokens:
            return ""
        content = " ".join(t.content_markdown for t in tokens)
    ...
    def _generate_markdown(
        ...
        if extract_toc:
            markdown_parts.append(self._get_table_of_contents(vgt_segments))
        ...
        for page in pdf_features.pages:
            ...
            for segment in segments_in_page:
                ...
                if segment.type == TokenType.PICTURE:
                    ...
                    markdown_parts.append(self._process_table_segment(segment))
                elif segment.type in {TokenType.TITLE, TokenType.SECTION_HEADER}:
                    markdown_parts.append(self._process_title_segment(tokens_in_seg, segment))
